...
const styles: Record<State, string> = {
  ...
  wrong: "bg-rose-100 border-rose-500 text-rose-800",
};
...
export function AnswerChoice({ label, state, disabled, onClick }: Props) {
  return (
    <button
      ...
      disabled={disabled}
      onClick={onClick}
      className={`w-full rounded-2xl border-2 px-5 py-4 text-lg font-semibold capitalize shadow-sm transition disabled:cursor-default ${styles[state]}`}
    >
      {label}
    </button>
  );
